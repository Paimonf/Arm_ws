...
class STM32CommunicationNode(Node):
    def __init__(self):
        ...
        if not self.test_mode: 
            ...
            self.get_logger().info(" 测试中，动作服务器已启用")
        else:
    ...
    def init_serial_connection(self):
        """初始化串口连接"""
        for i in range(self.max_retries-3):
            try:
                ...
                if self.serial_conn.is_open: 
                    ...
                    if self.send_sync(): 
                        ...
                        self.get_logger().error(f"同步 失败{i+1}次")
                        continue
                else:
    ...
    def send_command(self, cmd, data):
        ...
        # 添加调试信息
        self.get_logger().debug(f"发送命令: {[hex(b) for b in message]}")
        
        try:
            with self.serial_lock: 
                bytes_written = self.serial_conn.write(message) 
                self.serial_conn.flush()   # 确保数据发送完成 
                self.get_logger().debug(f"已发送 {bytes_written} 字节")
            return True 
        except Exception as e:
    ...
    def read_response(self, timeout=0.2):
            ...
            try:
                ...
                with self.serial_lock: 
                    ...
                    if available > 0:
                        ...
                        self.get_logger().debug(f"收到 {len(data)} 字节数据: {[hex(b) for b in data]}")
                
                # 尝试从缓冲区解析完整消息 
                response = self.parse_buffer() 
                if response:
                    self.get_logger().debug(f"解析到响应: {[hex(b) for b in response]}")
                    return response
        ...
        self.get_logger().warn(f"读取响应超时，超时时间: {timeout}秒")
        return None 
    ...
    # """带重试机制发送命令"""
        ...
        for attempt in range(retries):
            try:
                # 添加调试信息
                self.get_logger().info(f"尝试发送命令 {cmd}, 尝试 {attempt+1}/{retries}")
                
                if not self.send_command(cmd, data):
                    self.get_logger().warn(f"发送命令失败，尝试 {attempt+1}/{retries}")
                    time.sleep(0.1)
                    continue
                ...
                if expected_response is not None:
                    ...
                    if response and response[0] == expected_response:
                        self.get_logger().info(f"命令 {cmd} 发送成功")
                        return True 
                        ...
                        self.get_logger().warn(f"未收到预期响应，尝试 {attempt+1}/{retries}")
                        time.sleep(0.1)
                else:
                    return True
