...
from blueberry_interfaces.msg import Berry, DetectedBerries, BerryPickComplete
...
class BerryDetectionNode(Node):
    def __init__(self):
        ...
        self.batch_berries = []  # 存储当前批次的蓝莓
        self.arm_running = False  # 批次采摘是否完成
        ...
        # 采摘完成反馈订阅
        self.pick_complete_sub = self.create_subscription(
            BerryPickComplete,
            '/berry_pick/complete',
            self.pick_complete_callback,
            10
        )
        
        # 发布话题
        self.detection_pub = self.create_publisher(Image, '/berry_detection_result', 10)
        self.berry_pub = self.create_publisher(DetectedBerries, '/detected_berries', 10)
        
        self.get_logger().info("Berry Detection Node initialized")
    ...
    def pick_complete_callback(self, msg):
        """处理采摘完成反馈"""
        # if msg.arm_running:
        self.get_logger().info("Batch picking complete, ready for next detection")
        self.arm_running = True
        self.batch_berries = []  # 清空当前批次
    ...
    def color_image_callback(self, msg):
        ...
        try:
            ...
            self.current_color_image = frame
            ...
            self.detected_berries = detections
            
            # 绘制检测结果
            annotated_frame = results[0].plot()  # Ultralytics自带的绘图方法
            ...
            if self.current_depth_image is not None:
                self.process_detections_3d(msg.header)
    ...
    def depth_image_callback(self, msg):
        ...
        try:
            ...
            # 如果有颜色图像和检测结果，处理3D位置
            if self.current_color_image is not None and self.detected_berries:
                self.process_detections_3d(msg.header)
                
        except Exception as e:
            self.get_logger().error(f"Error processing depth image: {str(e)}")

    def process_detections_3d(self, header):
        """处理检测结果并计算3D位置"""
        if not self.depth_info or not self.detected_berries or self.current_depth_image is None:
            return
        ...
        try:
            ...
            berries_msg.header = header
            berries_msg.batch_size = 0
            ...
            for berry in self.detected_berries:
                ...
                point_stamped.header.stamp = header.stamp
            ...
            # 按图像坐标排序（左上到右下）
            # 排序逻辑：先y坐标（从上到下），再x坐标（从左到右）
            detected_points.sort(key=lambda p: (p['image_coords'][1], p['image_coords'][0]))
            
            # 限制单次采摘数量
            max_berries = self.get_parameter('max_berries_per_batch').value
            selected_points = detected_points[:min(len(detected_points), max_berries)]
            
            # 存储当前批次
            self.batch_berries = selected_points
            # self.arm_running = False
            
            # 创建消息
            for point in selected_points:
                berry_msg = Berry()
                berry_msg.position = point['position']
                berry_msg.confidence = point['confidence']
                berry_msg.is_ripe = point['is_ripe']
                berries_msg.berries.append(berry_msg)
            
            berries_msg.batch_size = len(berries_msg.berries)
            
            # 发布消息
            self.berry_pub.publish(berries_msg)
            self.get_logger().info(f"Published batch of {len(berries_msg.berries)} berries for picking")
            
        except Exception as e:
            self.get_logger().error(f"Error processing 3D detections: {str(e)}")
